...
def url_pairs_to_remove(args, bucket_urls, url_doc):
    ...
    iteration = 0
    while len(bucket_urls) > 1 and (
        args.heuristic_iter == -1 or iteration != args.heuristic_iter
    ):
...
if __name__ == '__main__':
    ...
    if args.load_fingerprints is not None:
        for count_fp, fp_file_name in enumerate(args.load_fingerprints):
            print(f"Loading fingerprints from pickle file {fp_file_name}", flush=True)
    ...
    if args.inputs is not None:
        ...
        for input_file, key in zip(args.inputs[::2], args.inputs[1::2]):
            print(f' document processing {input_file} with key {key}', flush=True)
    ...
    if args.save_fingerprints is not None:
        print(
            f"Saving fingerprints to pickle file {args.save_fingerprints}",
            flush=True,
        )
