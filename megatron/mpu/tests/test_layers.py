...
def test_parallel_embedding(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing parallel embedding with model parallel size {tensor_model_parallel_size} ...'
        )
    ...
    print(
        f'   error in loss (parallel) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-12, f'error: {error}'

    torch.distributed.barrier()
    error = loss_vocab_parallel.sub(loss_original).abs()
    print(
        f'   error in loss (vocab parallel) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-12, f'error: {error}'
    ...
    print(
        f'   error in grad (parallel) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-12, f'error: {error}'
    ...
    print(
        f'   error in grad (vocab parallel) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-12, f'error: {error}'
...
def test_initialize_affine_weight(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing initialize_affine_weight with model parallel size: {tensor_model_parallel_size}'
        )
    tensor_model_parallel_size = mpu.get_tensor_model_parallel_world_size()
    ...
    torch.distributed.barrier()
    print(
        f'   column parallel max error (should be zero) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-6
    ...
    torch.distributed.barrier()
    print(
        f'   row parallel max error (should be zero) on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-6
...
def test_column_parallel_linear(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing ColumnParallelLinear with model parallel size: {tensor_model_parallel_size}'
        )
    tensor_model_parallel_size = mpu.get_tensor_model_parallel_world_size()
    ...
    torch.distributed.barrier()
    print(
        f'   error in dLdA on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-6
    ...
    torch.distributed.barrier()
    print(
        f'   error in dLdb on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-6
    ...
    torch.distributed.barrier()
    print(
        f'   error in dLdX on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 1.0e-6
...
def test_row_parallel_linear(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing RowParallelLinear with model parallel size: {tensor_model_parallel_size}'
        )
    tensor_model_parallel_size = mpu.get_tensor_model_parallel_world_size()
...
def test_parallel_self_attention(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing ParallelSelfAttention with model parallel size: {tensor_model_parallel_size}'
        )
    ...
    torch.distributed.barrier()
    print(f'   loss error on global rank {torch.distributed.get_rank()}: {error}')
    assert error < 5.0e-6
    ...
    torch.distributed.barrier()
    print(
        f'   weight gradient error on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 5.0e-6
    ...
    torch.distributed.barrier()
    print(
        f'   input gradient error on global rank {torch.distributed.get_rank()}: {error}'
    )
    assert error < 5.0e-6
...
def test_parallel_transformer_layer(tensor_model_parallel_size):
    ...
    if torch.distributed.get_rank() == 0:
        print(
            f'> testing ParallelTransformerLayer with model parallel size: {tensor_model_parallel_size}'
        )
    ...
    torch.distributed.barrier()
    print(f'   loss error on global rank {torch.distributed.get_rank()}: {error}')
    assert error < 5.0e-5, f'error: {error}'
